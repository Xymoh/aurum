...
const zh: Dictionary = {
  ...
  verdict: {
    ...
    erNote:
      "有 {chance} 的重塑结果会让你低于该角色所需的 {threshold}% 元素充能效率 — 少放一次元素爆发的损失，远超重塑换来的暴击词条。重塑时请将元素充能效率选为两个指定词条之一加以保护。该需求值只是粗略参考，会随队伍变化，请结合自己的循环判断。",
  ...
  explainer: {
    ...
    erP2:
      "因此，当一件圣遗物的充能确实在发挥作用时，它会被标记为「充能有风险」并附上独立概率 — 即会让你充能不足的重塑比例。这个数字与重塑概率并列显示，而不是混入其中，因为充能需求对每个角色只是一个粗略数值，而且取决于队伍：单雷的芙林斯所需远高于双雷。让一个近似值悄悄否决建议，不如把两个数字都摆出来，交由你自己权衡。",
