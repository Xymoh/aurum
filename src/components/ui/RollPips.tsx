 ...
 */
export function RollPips({ rolls, unknownCount = 0, muted = false, className = "" }: RollPipsProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-[3px] ${muted ? "opacity-40" : ""} ${className}`} aria-hidden="true">
      {rolls.map((share, i) => (
        <span key={i} className={`h-2.5 w-[3px] rounded-sm ${ROLL_TIER_BG[rollTier(share)]}`} />
      ))}
